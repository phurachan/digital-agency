...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const allowedTypes = [
      ...
      // Image types (for backwards compatibility)
      'image/jpeg', 'image/jpg', 'image/png', 'image/gif', 'image/webp'
    ]

    if (!allowedTypes.includes(file.type || '')) {
      throw createPredefinedError(API_RESPONSE_CODES.INVALID_INPUT, {
        message: 'Only video files (MP4, WebM, MOV) and image files (JPEG, PNG, GIF, WebP) are allowed'
      })
    }
    ...
    if (file.data.length > maxSize) {
      ...
      throw createPredefinedError(API_RESPONSE_CODES.INVALID_INPUT, {
        message: `File too large. Maximum size is ${maxSizeText}`
      })
    }

      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center py-4">
          <div class="flex items-center space-x-4">
            ...
            <h1 class="text-xl font-semibold text-gray-900">Edit Services Page</h1>
          </div>
...
const loadContent = async () => {
  try {
    const response = await $fetch(buildApiUrl(API_ENDPOINTS.CMS.SERVICES_CONTENT.GET))
...
const handleSubmit = async () => {
  ...
  try {
    ...
    await $fetch(buildApiUrl(API_ENDPOINTS.CMS.SERVICES_CONTENT.POST), {
      method: 'POST',
      body: submitData
    })
...
// SEO
useSeoMeta({
  title: `Edit Services Page | ${siteSettings.siteName} CMS`,
